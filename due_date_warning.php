...
?>
    <!-- filter -->
    <div class="per_title">
      <h2><?php echo __('Jatuh Tempo Hari Ini'); ?></h2>
    </div>
    <div class="infoBox">
        <?php echo __('Report Filter'); ?>
        <div style='color:red;'><?php echo __('Melaporkan peminjaman eksemplar yang harus dikembalikan hari ini atau akan menjadi terlambat besok. Hal ini bisa diingatkan melalui pesan WA. -- maseko --'); ?></div> 
...
<?php
    ...
    $overdue_criteria = ' (l.is_lent=1 AND l.is_return=0 AND ( (TO_DAYS(due_date)-TO_DAYS(\''.date('Y-m-d').'\')) BETWEEN 0 AND 0)) ';
    ...
    function showOverduedList($obj_db, $array_data)
    {
        ...
        $_buffer = '<div style="color: blue;" class="font-weight-bold">'.$member_name.' ('.$array_data[0].')';
        $_buffer .= '<div style="color: black;">'.$member_mail_address;
        ...
        while ($_title_d = $_title_q->fetch_assoc()) {
            $_buffer .= '<tr>';
            $_buffer .= '<td valign="top" width="5%">'.$_title_d['item_code'].'</td>';
            $_buffer .= '<td valign="top" width="30%">'.$_title_d['title'].'</td>';
            $_buffer .= '<td width="1%" style="white-space:nowrap">'.__('Tgl Pinjam').': '.$_title_d['loan_date'].'</td><td width="1%" style="white-space:nowrap">'.__('Tgl Kembali').': '.$_title_d['due_date'].'</td>';
            
          //  if $_title_d['due_date']=DateTime() {
          
            $_buffer .= '<td width="10%"><p>Mengingatkan via WA <a class="btn btn-sm btn-outline-primary" href="https://wa.me/'.$member_d[2].'?text=MENGINGATKAN%0a%0aHaaai...%0aSahabat Ruang Perpustakaan SD Marsudirini Yogyakarta, kami mengingatkan bahwa:%0a%0aNama: *'.$member_d[0].'*%0aada pinjaman buku dengan:%0aKode Barcode: *'.$_title_d['item_code'].'*%0aJudul: *' . $_title_d['title'] . '*%0a%0aSilakan mengembalikan buku ke perpustakaan pada:%0aHARI INI: ' . $_title_d['due_date'] .'%0a%0aJika BELUM SELESAI MEMBACA, silakan melakukan Perpanjangan Peminjaman dengan membawa kartu anggota ke Perpustakaan sebelum terlambat.%0a%0aTerima Kasih.%0aTuhan memberkati.%0a%0aNotifikasi ini tidak perlu dibalas.%0a%0a'.$sysconf['library_name'].'-'.$sysconf['library_subname'].'" target="_blank"> <i class="fa fa-paper-plane-o"> '.$member_d[2].''.$_title_d[3].'</a></p></td>';
